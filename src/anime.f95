...
subroutine anime(n,n0,nmax,km,jm,im,dxl,dx1,dyl,dy1,z2,data22,data23,u,w,v,amask1)
    ...
    if(n == n0.or.n == nmax.or.mod(n,1000) == 0.) then
        ...
        write(23) im,jm,km,4
        write(23) (((real(0.5*(u(i-1,j,k)+u(i,j,k))), i=1,im),j=1,jm),k=1,km), &
                  (((real(0.5*(w(i,j, k-1)+w(i,j,k))), i=1,im),j=1,jm),k=1,km), &
                  (((real(0.5*(v(i,j-1,k)+v(i,j,k))), i=1,im),j=1,jm),k=1,km), &
                  (((real(amask1(i,j,k)),i=1,im),j=1,jm),k=1,km)
        close(unit=23)
    endif
